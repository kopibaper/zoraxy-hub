...
export async function GET(
  ...
  try {
    ...
    if (connector.supportsFileAccess?.() === false || !connector.readConfigFile) {
    ...
    const content = await connector.readConfigFile(path);
    return Response.json({ success: true, data: { path, content } });
  } catch (error) {
